...
module.exports = class RCTree extends Tree {
  /**
   * (Marathon) RCTree
   * @param {{
   *          id:string,
  ...
  constructor(options = {}) {
    ...
    // Converts items into instances of RCTree, Application or Framework
    // based on their properties.
    this.list = this.list.map((item) => {
      if (item instanceof RCTree) {
        return item;
      }

      // Check item properties and convert items with an items array or an apps
      // and groups array (Marathon group structure) into RCTree instances.
  ...
  /**
   * @param {string} id
   * @return {Service|RCTree} matching item
   */
  findItemById(id) {
