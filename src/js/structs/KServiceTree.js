...
module.exports = class KServiceTree extends Tree {
  /**
   * (Marathon) KServiceTree
   * @param {{
   *          id:string,
  ...
  constructor(options = {}) {
    ...
    // Converts items into instances of KServiceTree, Application or Framework
    // based on their properties.
    this.list = this.list.map((item) => {
      if (item instanceof KServiceTree) {
        return item;
      }

      // Check item properties and convert items with an items array or an apps
      // and groups array (Marathon group structure) into KServiceTree instances.
  ...
  /**
   * @param {string} id
   * @return {Service|KServiceTree} matching item
   */
  findItemById(id) {
  ...
  filterItemsByFilter(filter) {
    ...
    return new this.constructor(Object.assign({}, this, {items: rcs}));
  }
